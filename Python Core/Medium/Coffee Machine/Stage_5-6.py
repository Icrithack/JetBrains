class CoffeeMachine:
    ...
    def check_ingredients(self, action):
        temp = [[self.water - 250, self.coffee - 16, self.disposable_cups - 1],
                [self.water - 350, self.milk - 75, self.coffee - 20, self.disposable_cups - 1],
                [self.water - 200, self.milk - 100, self.coffee - 12, self.disposable_cups - 1]
                ]

        for i in temp[int(action)-1]:
            if i < 0:
                return False
            else:
                return True
    ...
    def main(self):
        while True:
            ...
            if action == 'buy':
                ...
                if coffee_type == 'back':
                    continue
                if self.check_ingredients(coffee_type):
                ...
                print(f'Action "{action}" do not exist.')
